...
# Profesionální legenda
leg_sig_root = ROOT.TLegend(0.45, 0.65, 0.90, 0.88)
leg_sig_root.SetBorderSize(0)
leg_sig_root.SetFillStyle(0)
...
txt_opt = ROOT.TLatex(0.18, 0.82, f"Optimal window: #Delta m = {windows_arr[opt_exp_idx]:.1f} GeV")
txt_opt.SetNDC(True)
...
ax.set_xlabel(r"Mass window full width $\Delta m$ [GeV] (around $125\text{ GeV}$)")
...
lbl_max = f"Optimum L=1.0: $\\Delta m = {windows_arr[opt_exp_idx]:.1f}\\text{{ GeV}}$\\n$Z = {z_exp_arr[opt_exp_idx]:.2f}\\sigma$"
ax.annotate(lbl_max,
...
txt_sb_root = ROOT.TLatex(0.20, 0.80, "Best fit: #hat{#alpha}_{bgr} = %.3f #pm %.3f" % (best_alpha, sigma_alpha))
txt_sb_root.SetNDC(True)
txt_sb_root.SetTextFont(42)
txt_sb_root.SetTextSize(0.042)
txt_sb_root.Draw()

txt_l1 = ROOT.TLatex(0.20, 0.22, "1#sigma CL (#Delta(-2 ln L) = 1.0)")
txt_l1.SetNDC(True)
txt_l1.SetTextFont(42)
txt_l1.SetTextSize(0.032)
txt_l1.SetTextColor(ROOT.kRed)
txt_l1.Draw()

txt_l2 = ROOT.TLatex(0.20, 0.52, "2#sigma CL (#Delta(-2 ln L) = 4.0)")
txt_l2.SetNDC(True)
txt_l2.SetTextFont(42)
txt_l2.SetTextSize(0.032)
txt_l2.SetTextColor(ROOT.kMagenta + 2)
txt_l2.Draw()
...
ax.set_xlabel(r"Background scale factor $\alpha_{\text{bgr}}$")
...
lbl_fit = f"Best fit:\\n$\\hat{{\\alpha}}_{{\\text{{bgr}}}} = {best_alpha:.3f} \\pm {sigma_alpha:.3f}$"
ax.annotate(lbl_fit,
...
lbl_best = f"Best Fit ($\\hat{{\\alpha}} = {best_alpha_2d:.2f}, \\hat{{\\mu}} = {best_mu_2d:.2f}$)"
...
ax.set_xlabel(r"Background scale factor $\alpha_{\text{bgr}}$")
ax.set_ylabel(r"Signal strength modifier $\mu_{\text{sig}}$")
ax.set_xlim(0.8, 1.5)
ax.set_ylim(0.2, 2.5)
