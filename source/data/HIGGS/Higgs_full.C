...
//MIRA   Note: Histograms have ~200 MeV bins, so need to rebin  PRESNE: 0.205556 GeV, od 60 GeV

//MIRA   h_data->Integral(294,348)  = 16.0000
...
//=============================================================
  ...
  for (int i_bin = 1; i_bin<h_masswindow->GetNbinsX(); i_bin++ ){
    ...
    double Ndata_win = h_data -> Integral(h_data->FindBin(125 - masswindow_fullwidth/2.),h_data->FindBin(125 + masswindow_fullwidth/2.));
    double Nbgr_win = h_bgr -> Integral(h_data->FindBin(125 - masswindow_fullwidth/2.),h_data->FindBin(125 + masswindow_fullwidth/2.));
    double Nsig_win = h_sig -> Integral(h_data->FindBin(125 - masswindow_fullwidth/2.),h_data->FindBin(125 + masswindow_fullwidth/2.));
	...
	double maximum_obs = h_masswindow_observed->GetBinCenter(h_masswindow_observed->GetMaximumBin());
	double maximum_exp = h_masswindow_expected->GetBinCenter(h_masswindow_expected->GetMaximumBin());

	printf("max expected significance %f, max observed significance %f \n", maximum_exp,maximum_obs);
...
void Fit1DLikelihood(int Irebin)
{
  printf("\n SideBandFit()\n\n");
  ...
  h_bgr  = GetMassDistribution(1);
  h_data = GetMassDistribution(2);
  h_sig  = GetMassDistribution(125,1);
  ...
  double scalefactor_bgr  = 1.105; // from the exercise 2 

  TH1D *h_scalefactor_sig = new TH1D("h_scalefactor_sig","",100,0.1,5.);// likelihood distribution
  double scalefactor_sig  = 1.; // from the exercise 2 

  //---------------------------------------------
  // [2a] Loop 1: loop over scalefactors in alpha
  //---------------------------------------------
  for (int i_bin_higgs = 1; i_bin_higgs <=h_scalefactor_sig->GetNbinsX(); i_bin_higgs++){

    //-- determine the scale factor for the background
    scalefactor_sig = h_scalefactor_sig->GetBinCenter(i_bin_higgs);
    ...
    //-----------------------------------------------------------------------------------
    double loglik = 1e-10;
    for (int i_bin = h_data->FindBin(0); i_bin <=  h_data->FindBin(400); i_bin++){
     // printf("        bin %d, m = %5.2f, Ndata = %5.2f, Nbgr = %5.2f\n",i_bin, h_bgr->GetBinCenter(i_bin),h_bgr->GetBinContent(i_bin),h_data->GetBinContent(i_bin));
      loglik += TMath::Log(TMath::PoissonI(h_data->GetBinContent(i_bin),scalefactor_sig*h_sig->GetBinContent(i_bin) + scalefactor_bgr*h_bgr->GetBinContent(i_bin)));   	
    } // end loop over bins

    h_scalefactor_sig->SetBinContent(i_bin_higgs,-2.*loglik);   
    
  } // end loop over scale factors for the background

  h_scalefactor_sig->Draw();

  double min_loglik = h_scalefactor_sig->GetBinCenter(h_scalefactor_sig->GetMinimumBin());


    TH1D *h_scalefactor_sig_rescaled  = (TH1D*) h_scalefactor_sig->Clone("h_scalefactor_sig_rescaled");  
    //h_scalefactor_bgr_rescaled->Reset();

    h_scalefactor_sig_rescaled->Scale(-1);

    //h_scalefactor_bgr->Draw();
	
	//--Rescale and find +/- 1 sigma errors
    double lower_error = h_scalefactor_sig_rescaled->GetBinCenter(h_scalefactor_sig_rescaled->FindFirstBinAbove(h_scalefactor_sig_rescaled->GetMaximum() -1)
    	);   
    double upper_error = h_scalefactor_sig_rescaled->GetBinCenter(h_scalefactor_sig_rescaled->FindLastBinAbove( h_scalefactor_sig_rescaled->GetMaximum() -1)); 
...
void Fit2DLikelihood(int Irebin)
{
  ...
  TH2D *h_scalefactor = new TH2D("h_scalefactor","",200,0.1,3.01,200,0.1,3.01);// likelihood distribution
  double scalefactor_sig  = 1.; 
  double scalefactor_bgr  = 1.;

  //---------------------------------------------
  // [2a] Loop 1: loop over scalefactors in alpha
  //---------------------------------------------

  for(int i_bin_sm = 1; i_bin_sm <=h_scalefactor->GetNbinsX(); i_bin_sm++){ //loop over x (=bgr)
  	for (int i_bin_higgs = 1; i_bin_higgs <=h_scalefactor->GetNbinsY(); i_bin_higgs++){ // loop over y (=sig)
  	  double  scalefactor_bgr=h_scalefactor->GetXaxis()->GetBinCenter( i_bin_sm );
  	  double  scalefactor_sig=h_scalefactor->GetXaxis()->GetBinCenter( i_bin_higgs );

  	  //printf("alfa = %f   mu = %f\n", scalefactor_bgr,scalefactor_sig);

      double loglik = 1e-10;
    
         for (int i_bin = 1; i_bin <=  h_data->GetNbinsX(); i_bin++){
      	    loglik += TMath::Log(TMath::PoissonI(h_data->GetBinContent(i_bin),
      	    	scalefactor_sig*h_sig->GetBinContent(i_bin) + scalefactor_bgr*h_bgr->GetBinContent(i_bin)));   	
    	 } // end loop over bins
  ...
  int minimumSFbin,minimumAlphabin,value;
  h_scalefactor->GetBinXYZ(h_scalefactor->GetMinimumBin(), minimumSFbin, minimumAlphabin,value);
  cout << h_scalefactor->GetXaxis()->GetBinCenter(minimumSFbin) << " "<<h_scalefactor->GetYaxis()->GetBinCenter(minimumAlphabin)<<endl;


  TH1D * h_scalefactor_Xprojection = h_scalefactor->ProjectionX("",minimumAlphabin,minimumAlphabin);
  TH1D * h_scalefactor_Yprojection = h_scalefactor->ProjectionY("",minimumSFbin   ,minimumSFbin   );

  TH1D *h_scalefactor_Xprojection_rescaled  = (TH1D*) h_scalefactor_Xprojection->Clone("h_scalefactor_Xprojection_rescaled");  
  TH1D *h_scalefactor_Yprojection_rescaled  = (TH1D*) h_scalefactor_Yprojection->Clone("h_scalefactor_Xprojection_rescaled");  

  h_scalefactor_Xprojection_rescaled->Scale(-1);
  h_scalefactor_Yprojection_rescaled->Scale(-1);

  h_scalefactor_Yprojection_rescaled->Draw();

  double lower_errorX = h_scalefactor_Xprojection_rescaled->GetBinCenter(h_scalefactor_Xprojection_rescaled->FindFirstBinAbove(h_scalefactor_Xprojection_rescaled->GetMaximum() -1));   
  double upper_errorX = h_scalefactor_Xprojection_rescaled->GetBinCenter(h_scalefactor_Xprojection_rescaled->FindLastBinAbove( h_scalefactor_Xprojection_rescaled->GetMaximum() -1));

  double lower_errorY = h_scalefactor_Yprojection_rescaled->GetBinCenter(h_scalefactor_Yprojection_rescaled->FindFirstBinAbove(h_scalefactor_Yprojection_rescaled->GetMaximum() -1));   
  double upper_errorY = h_scalefactor_Yprojection_rescaled->GetBinCenter(h_scalefactor_Yprojection_rescaled->FindLastBinAbove( h_scalefactor_Yprojection_rescaled->GetMaximum() -1));

  double XX = h_scalefactor->GetXaxis()->GetBinCenter(minimumSFbin);
  double YY = h_scalefactor->GetYaxis()->GetBinCenter(minimumAlphabin);

  printf("scalefactor1 = %f + %f - %f \n", XX, upper_errorX - XX,-lower_errorX + XX);

  printf("scalefactor2 = %f + %f - %f \n", YY, upper_errorY - YY,-lower_errorY + YY);
...
//===========================
  ...
  for (int i_bin_sf = 1; i_bin_sf <=h_scalefactor_bgr->GetNbinsX(); i_bin_sf++){
    ...
    for (int i_bin = h_data->FindBin(150); i_bin <=  h_data->FindBin(400); i_bin++){
     // printf("        bin %d, m = %5.2f, Ndata = %5.2f, Nbgr = %5.2f\n",i_bin, h_bgr->GetBinCenter(i_bin),h_bgr->GetBinContent(i_bin),h_data->GetBinContent(i_bin));
      loglik += TMath::Log(TMath::Poisson(h_data->GetBinContent(i_bin),scalefactor_bgr*h_bgr->GetBinContent(i_bin)));   	
    } // end loop over bins
...
//    h_scalefactor_bgr_rescaled->Reset();
    h_scalefactor_bgr_rescaled->Scale(-1);
    h_scalefactor_bgr_rescaled->Draw();

    //-- Find minimum
    double min_loglik = h_scalefactor_bgr_rescaled->GetBinCenter(h_scalefactor_bgr->GetMinimumBin());

	
	//--Rescale and find +/- 1 sigma errors
    double upper_error = h_scalefactor_bgr_rescaled->GetBinCenter(h_scalefactor_bgr_rescaled->FindFirstBinAbove(h_scalefactor_bgr_rescaled->GetMaximum() -1)
    	);   
    double lower_error = h_scalefactor_bgr_rescaled->GetBinCenter(h_scalefactor_bgr_rescaled->FindLastBinAbove( h_scalefactor_bgr_rescaled->GetMaximum() -1)); 

   	printf("scalefactor = %f + %f - %f \n", min_loglik, upper_error - min_loglik,-lower_error+min_loglik);

	double masswindow = 7.2/2.; // got fvrom the 1st exercise

	double N_bgr125 = h_bgr->Integral(h_bgr->FindBin(125. - masswindow),h_bgr->FindBin(125. + masswindow));
	double N_bgr125_low = N_bgr125*lower_error;
	double N_bgr125_upp = N_bgr125*upper_error;
	double N_bgr125_sca = N_bgr125*min_loglik;

	printf("number of background before scale = %f\n", N_bgr125);
	printf("number of background  after scale = %f + %f - %f \n", N_bgr125_sca, N_bgr125_upp-N_bgr125_sca,N_bgr125_sca-N_bgr125_low);


	TH1D *h_bgr_generated  = (TH1D*) GenerateToyDataSet(h_bgr,min_loglik,upper_error - min_loglik,-lower_error+min_loglik,N_bgr125_sca)->Clone("h_bgr_generated");
//MIRA	h_bgr_generated->Draw();

	TH1D *h_sig;
	h_sig  = GetMassDistribution(125, 1);

    double Nsig = h_sig-> Integral(h_sig->FindBin(125. - masswindow),h_sig->FindBin(125. + masswindow));
    double Nbgr_before = h_bgr-> Integral(h_bgr->FindBin(125. - masswindow),h_bgr->FindBin(125. + masswindow));
 


    double pvalue_expected_before = IntegratePoissonFromRight(N_bgr125_sca,(int)(N_bgr125_sca+Nsig)) ;
    double pvalue_expected_now    = IntegrateFromRight(h_bgr_generated,N_bgr125_sca+Nsig) ;
    double significance_expected_before = ROOT::Math::gaussian_quantile_c(pvalue_expected_before,1);
    double significance_expected_now    = ROOT::Math::gaussian_quantile_c(pvalue_expected_now   ,1);



	printf("expected significance begore: %f \n", significance_expected_before);
	printf("expected significance now   : %f \n"   , significance_expected_now   );
	//Significance_Optimization();
...
//===============================================
  ...
  double masswindow = 7.2/2.; // from exercise 1
  	
  for (int i = 0; i< 150000; i++){
  	double N_generated = R->PoissonD(R->Gaus(N_bgr,0.06));// (lower+upper)/2.)
