...
float chi2(TH1D* hdata, float a, float b, TF1* f1)
{
	...
	for(int i=1; i<=npoints; i++) //loop over histogram bins
	{
		...
		float g	   = f1->Eval(x);
		float diff = (g - n);
		chi2 += (n>0) ? (diff*diff)/n : 0;  //make sure we don't divide by zero
//		chi2 += (g>0) ? (diff*diff)/g : 0;  //make sure we don't divide by zero

	// a nebo zap. cislem! ... tim ze odecitame pozadi ... jinak to nema smysl, mela by to byt min 0
